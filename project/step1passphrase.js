...
//Verify that it is a 12 word phrase and a valid passphrase
//If 12 words but not valid, suggest
//If valid, print a list of addresses and private keys plus relevant links

numAddresses = 12; //# of addresses to display from passphrase
timeoutID = 0;
function preparePP() {
    clearTimeout(timeoutID);
    timeoutID = setTimeout(validateInputPP, 70);
}
...
function validateInputPP() {
    ...
    var ppSuggest = repairPP(ppClean);
    if (isValidPP(ppSuggest)) {
        output += "<pre>";
        output += "PASSPHRASE";
        output += "<br>";
        output += highlightDiffWords(ppSuggest, ppClean);
        output += "<br>";
        output += "<br>";
        output += fixedlength("#",3,false);
        output += fixedlength("ADDRESS",36,false);
        output += fixedlength("PRIVATE KEY",54,false);
        
        var words = ppSuggest.split(' ');
        var addresses = genAddr(ppSuggest,numAddresses,true);
        for (var i = 0; i < numAddresses; i++) {
            var address = addresses[i][0];
            var privkey = addresses[i][1];
            output += "<br>";
            output += fixedlength(i+1,3,false);
            output += fixedlength(address,36,false);
            output += fixedlength(privkey,54,false);
            output += "<a href=\"http://blockscan.com/address/"+address+"\">BS</a>\xA0\xA0";
            output += "<a href=\"https://counterpartychain.io/address/"+address+"\">CC</a>\xA0\xA0";
            output += "<a href=\"https://blockchain.info/address/"+address+"\">BC</a>\xA0\xA0";
            output += "<a href=\"https://www.blocktrail.com/BTC/address/"+address+"/transactions\">BT</a>\xA0\xA0";
        }
	   output += "</pre>";
	}
...
function ppStringToIntArray(passphrase) {
	//Converts string to integer array
	var words = passphrase.split(' ');
	var intArray = [];
	for (var i = 0; i < words.length; i++) {
		intArray[i] = ppDictionary.indexOf(words[i]);
	}
	return intArray;
}

function ppIntArrayToString(intArray) {
	//Converts integer array to string
	var passphrase = ''; 
	for (var i = 0; i < intArray.length; i++) {
		passphrase += ppDictionary[intArray[i]] + ' ';
	}
	passphrase = passphrase.trim();
	return passphrase;
}

function repairPP(inputpp) {
	//Get Levenstein distance for each input word to closest ppDictionary word.
	//This is an integer; 0 if word in ppDictionary (what we want), 1 if very close (e.g. short instead of shirt), 
	//and higher if less similar.
	var inputwords = inputpp.split(' ');
	var levDis = [];
	var closest = [];
	for (var i = 0; i < inputwords.length; i++) {
		levDis[i] = levDist(inputwords[i],ppDictionary[0]);
		//Special case. Prioritize if all chars of shortest string is in longest. Useful when more than one best fit. 
		if ((levDis[i] == 1 || levDis[i] == 2) && ppDictionary[0].length != inputwords[i].length) {
			if (allSameChars(ppDictionary[0], inputwords[i])) levDis[i] -= 0.5;
		}
		closest[i] = 0;
		for (j = 1; j < ppDictionary.length; j++) {
			var levDisTemp = levDist(inputwords[i],ppDictionary[j]);
			
			//Special case. Prioritize one where all chars of shortest string is in longest
			if ((levDisTemp == 1 || (levDis[i] >= 2 && levDisTemp == 2)) && ppDictionary[j].length != inputwords[i].length) {
				if (allSameChars(ppDictionary[j], inputwords[i])) levDisTemp -= 0.5;
			}
	...
	//Repair by replacing words not found in ppDictionary with closest match in ppDictionary (if no close match; "???")
	var repairedpp = '';
	for (var i = 0; i < inputwords.length; i++) {
		if (levDis[i] <= 1 || (levDis[i] <= 2 && inputwords[i].length >= 5 && ppDictionary[closest[i]].lenght >= 5)) {
			repairedpp += ppDictionary[closest[i]] + ' ';
		} else {
			repairedpp += '??? ';
		}
	}
...
var levDist = function(s, t) {
	...
	//http://www.mgilleland.com/ld/ldjavascript.htm, Damerau–Levenshtein distance
	var d = []; //2d matrix
...
function allSameChars(word1, word2) {
	...
	for (var k = 0; k < shortestChars.length; k++) {
...
function genAddr(passphrase, numOutputs, privkeys) {
	...
	//For this to work, bitcore must be loaded: <script src="lib/bitcore/bitcore.min.js"></script>
...
//generating addresses requires bitcore
function genAddr(passphrase, numOutputs, privkeys) {
	//Returns 2d array with addresses and private keys
	//For this to work, bitcore must be loaded: <script src="lib/bitcore/bitcore.min.js"></script>
	if (typeof numOutputs === 'undefined') { numOutputs = 12; }
