...
//return true if asset is divisible, false if not (then only integer amounts)
//this function MAY have to make an API call. Then it will delay execution of entire script by ~0.2-1 sec
//in rare cases when array lookup and API call fail it will return -1 (very important to abort any sends then) 
	if (asset == "XCP") return true;
	var indAsset = assetListIndex(asset);
	var divisible = true;
	if (indAsset != -1) {
		if (ASSET_LIST[indAsset][3] == 0) return false;	
		if (ASSET_LIST[indAsset][3] == 1) return true;	
	} else {
		//API call
		var jsondata = jsonFromURL("https://xchain.io/api/asset/" + asset);
		if (jsondata != "-1") {
			if(jsondata.hasOwnProperty('divisible')) {
				if (jsondata['divisible'] == 0) return false;
				if (jsondata['divisible'] == 1) return true;
			} else {
