...
//****
//SEND ASSET (input priv key)
// Copy of above SEND ASSET funcions except takes private key input (not mnemonic)
// TODO : make standard functions take either one
var sendXcpOK = 0;
var sendXcpStatus = "";
var sendTimeouts = [];
function send_piggyback_privkey(add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv) {
    sendXCP_opreturn_privkey(add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv, false);
}
function sendXCP_opreturn_privkey(add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv, check_asset_balance) {
	hex_byte();    
	bitcore = require('bitcore');
	if (typeof(feedbackdiv)==='undefined') feedbackdiv = "sendfeedback";
	if (typeof(check_asset_balance)==='undefined') check_asset_balance = true;
	sendXcpOK = 0;
	var asset_total_input = asset_total;
	var divisibility = isDivisible(asset);
	...
	//Error tests
	if (divisibility == -1) {
		document.getElementById(feedbackdiv).innerHTML = "<span style=\"color:DarkRed;font-weight:bold;\">XChain API LOOKUP FAILED</span><br />Cannot send.";
		return;
	}
	if (check_asset_balance && getBalance(add_from,asset) < asset_total_input) {
		document.getElementById(feedbackdiv).innerHTML = "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />Address balance is too low.";
	...
	if (transfee > 0.0099) { //a hardcoded test to prevent accidentally high BTC fee (today ~$25)
		document.getElementById(feedbackdiv).innerHTML = "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />BTC fee is "+transfee.toFixed(8)+" and that's too high, right?";
		return;
	}
	...
	sendXcpStatus += "Sending...";
	document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
	sendTimeouts.push(setTimeout(prepareSendXCP_opreturn_privkey_bitpay, 1, add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv));
	sendTimeouts.push(setTimeout(prepareSendXCP_opreturn_privkey_blockchaininfo, API_TIMEOUT_MS, add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv));
	sendTimeouts.push(setTimeout(writeSendStatus_privkey, API_TIMEOUT_MS*2, feedbackdiv));        
}

function prepareSendXCP_opreturn_privkey_bitpay(add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv) {
	if (sendXcpOK != 1) { 
		sendXcpStatus += "<br>Trying Bitpay API"; 
		sendXCP_opreturn_privkey_bitpay(add_from, add_to, asset, asset_total, btc_total, transfee, privkey);
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
	} else {
		writeSendStatus_privkey(feedbackdiv);
	}
}
function prepareSendXCP_opreturn_privkey_blockchaininfo(add_from, add_to, asset, asset_total, btc_total, transfee, privkey, feedbackdiv) {
	if (sendXcpOK != 1) { 
		sendXcpStatus += "<br>Trying Blockchain.info API"; 
		sendXCP_opreturn_privkey_blockchaininfo(add_from, add_to, asset, asset_total, btc_total, transfee, privkey);
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
	} else {
		writeSendStatus_privkey(feedbackdiv);
	}
}
function writeSendStatus_privkey(feedbackdiv) {
	for (var i=0; i<sendTimeouts.length; i++) {
		clearTimeout(sendTimeouts[i]);
	}
    if (isPushingTx) {
        setTimeout(writeSendStatus_privkey, 100, feedbackdiv);
        return;
    } else if (sendXcpOK == 0) 	{ 
		sendXcpStatus += "<br><span style=\"color:DarkRed ;font-weight:bold;\">SEND FAILED</span><br />Either fail across all APIs, no Internet connection, or insufficient BTC."; 
	} else if (pushTxOK == 0) {
...
function sendXCP_opreturn_privkey_bitpay(add_from, add_to, asset, asset_total, btc_total, transfee, privkey) {
    var buildTxStart = new Date().getTime(); 
	console.log("bitpay api start");	
    //var mnemonic = $("#newpassphrase").html();
...
function sendXCP_opreturn_privkey_blockchaininfo(add_from, add_to, asset, asset_total, btc_total, transfee, privkey) {
    var buildTxStart = new Date().getTime();
	console.log("blockchain.info api start");	
    //var mnemonic = $("#newpassphrase").html();
